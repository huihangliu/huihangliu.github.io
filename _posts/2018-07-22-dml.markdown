...
Ref: [jmarkhou's Blog](http://jmarkhou.com/dml/)
...
$$\sqrt{n} \mathbb{E}_n[\partial_{\eta^2}\phi(X, \theta_0, \eta_0)[\hat{\eta}_n-\eta_0]]$$ to both go to 0.  Here's where Neyman orthogonality and cross-fitting come in:
...
2. Cross-fitting + assumption that $$\lvert\lvert\hat{\eta}-\eta_0\rvert\rvert_2 = o_p(n^{-1/4})$$ on hold-out data gives us $$\sqrt{n} \mathbb{E}_n[\partial_{\eta^2}\phi(X, \theta_0, \eta_0)[\hat{\eta}-\eta_0]]$$ going to 0
